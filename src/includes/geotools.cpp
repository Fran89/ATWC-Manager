...
/* Check inputs, if they coud cause floating point error, send back zeroes */
   iFlag = 1;
   if ( pll1->dLat <= 0. )
   {
...
const char *GetOpAgency( char *pszNetID )
{
...
const char *GetSeisInfo( int iSeisType, double *dFlatResp )
{
   *dFlatResp = 1.;
   if ( iSeisType == 103 )
...
double LLConv( char *psz )
{
   ...
   return dTemp;
       ...
       *   iFormat          Flag showing what fieds this file contains *
...
/* For parsing, pad szLine with blanks (-1 for EOL char) */
         for ( j=strlen( szLine )-1; j<456; j++ ) szLine[j] = ' ';
         szLine[456] = '\0';           
...
/* Create long name in CAPS */
            ...
            {                                                    /* Site Name */
               ttt[i].szSiteNameCapLong[jCnt] = toupper( ttt[i].szSiteName[j] );
               jCnt++;
            }
            ...
            {                                                   /* Site State */
               ttt[i].szSiteNameCapLong[jCnt] = toupper( ttt[i].szSiteState[j] );
               jCnt++;
            }
            ...
            {                           
               ttt[i].szSiteNameCapLong[jCnt] = toupper( szRelLoc[j] );
               jCnt++;
            }
            if ( (int) strlen( szRelLoc ) > 0 )
            {
...
/* Compute iWWType here */         
   ...
   return( iInFile );
       ...
       * or advisory shoud be called for this region.  It is only      *
